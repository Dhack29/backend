...
router.post('/from-segment', async (req, res) => {
  try {
    const { segmentId, name, description, messageTemplate, messageContent } = req.body;
    ...
    const campaign = new Campaign({
      ...
      description,
      segment: segmentId,
      messageTemplate: messageTemplate || 'Hi {{customerName}}, {{message}}',
      messageContent,
      status: 'draft',
      stats: {
...
router.get('/:id/logs', async (req, res) => {
  try {
    ...
    // Also get campaign status
    const campaign = await Campaign.findById(req.params.id);
    
    res.json({
      logs,
      campaignStatus: campaign ? campaign.status : null,
      stats: campaign ? campaign.stats : null
    });
  } catch (error) {
    console.error('Error fetching campaign logs:', error);
    res.status(500).json({ error: 'Error fetching campaign logs' });
  }
});
...
router.get('/campaigns/:campaignId/logs', async (req, res) => {
  try {
